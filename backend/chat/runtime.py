...
def _completed_persisted_history(
    ...
    *,
    ...
    for index, assistant in enumerate(rows):
        ...
        user_turn = str(user.get("turn_id") or "")
        if current_turn_id and assistant_turn == current_turn_id:
            continue
        if assistant_turn and user_turn and assistant_turn != user_turn:
            continue
...
def build_basic_messages(
    *,
    ...
    history = _completed_persisted_history(
        persisted_messages,
        ...
        with run_control.track_phase("generation", agent_id="basic-chat", model=model):
            ...
            if int(response.status_code) != 200:
                state.failure = model_call_error(
                    settings, model, int(response.status_code), str(response.text or "")
                )
                return
...
def _persist_failed_run(
    *, run_id: str, session_id: str, turn_id: str, model: str,
    ...
    database.upsert_run(
        ...
        metrics={"runtime": "basic_chat", **(extra_metrics or {}), **({"error": failure} if failure else {})},
...
def _persist_completed_run(
    *, run_id: str, session_id: str, turn_id: str, model: str,
    ...
    database.add_message(
        ...
        sources=[], process_events=[], artifacts=[], turn_id=turn_id,
    ...
    database.upsert_run(
        ...
        tasks=[], events=[], sources=[], metrics=metrics, artifacts=[], completed_at=_now_iso(),
    )
...
async def stream_basic_chat(
    ...
    database.upsert_run(
        ...
        tasks=[], events=[], sources=[], artifacts=[],
    )
    yield _meta_event(binding, model)
    ...
    try:
        ...
        if state.failure:
            _persist_failed_run(
                ...
                model=model, failure=state.failure,
            )
            yield encode_sse("error", {**state.failure, "content": state.failure.get("message")})
            return
        run_control.raise_if_cancelled_or_expired()
        ...
        _persist_completed_run(
            ...
            metrics=metrics, archive_sync=archive_sync,
        )
        ...
        failure = None if cancelled else model_transport_error(settings, model, exc)
        _persist_failed_run(
            run_id=run_id, session_id=session_id, turn_id=turn_id, model=model,
        ...
        yield encode_sse(event, {**(failure or binding), "content": message, "message": message})
